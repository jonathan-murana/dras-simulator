...
public class ____HEU_FF_AT extends HEU {
	...
	public static int [] schedule(
		  ...
		  int [] task_sorted_by_at = Utils.sort_by_min(problem.getAT());
			  ...
			  for (int i :Arrays.stream(task_sorted_by_at).boxed().collect(Collectors.toList())) {
				  ...
				  for (int r = 0 ; r < problem.getR() ; r++ ) {
						  ...
						  if  (isSuitable(problem,i,r,currect_execution_time) && assigned_task[i] == 0) {
							  ...
							  if (!isFullForTask(problem,i, r, k, task_duration, task_steps, server_usage_by_steps_cpu, 
